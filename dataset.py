...
class ProteinECFPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor(label, dtype=torch.long)
        protein = self.protein_encoder.encode(bytes(protein, encoding='utf8'))
        tokens, segments, input_mask, length = self._preprocess(protein)

        ecfp_tensor = torch.tensor([int(c) for c in chem], dtype=torch.float32)

        return tokens, segments, input_mask, length, label, ecfp_tensor
